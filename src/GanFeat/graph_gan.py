...
class GraphGan(object):
    def __init__(self):
        ...
        self.generator = None
        self.maxSoftmaxGen=0.0
        self.maxSoftmaxDis=0.0
        self.maxMedianGen=0.0
        self.maxMedianDis=0.0
        ...
        self.node_feature_init = utils.read_emd(filename=config.pretrain_feature_filename, n_node=config.n_node, n_embed=config.n_features)
        
        print("finish reading initial embeddings")
        # us the BFS to construct the trees
    ...
    def build_discriminator(self):
        ...
        with tf.variable_scope("discriminator"):
            self.discriminator = discriminator.Discriminator(n_node=self.n_node, node_emd_init=self.node_embed_init_d, node_features = self.node_feature_init)

    def build_generator(self):
        """initialize the generator"""

        with tf.variable_scope("generator"):
            self.generator = generator.Generator(n_node=self.n_node, node_emd_init=self.node_embed_init_g,  node_features = self.node_feature_init)
    ...
    def train_gan(self):
        ...
        self.write_emb_to_txt(0,4)
        self.write_emb_to_txt(1,5)
        ...
        for epoch in tqdm.tqdm(range(config.max_epochs)):
                ...
                for root_node in tqdm.tqdm(self.root_nodes, mininterval=3):  # random update trees
                    if np.random.rand() < config.update_ratio:
                        ...
                        if cnt % config.gen_update_iter == 0 and cnt > 0:
                            ...
                            reward_gen, node_embed = self.sess.run([self.discriminator.reward, self.discriminator.node_embed],
                                                       {self.discriminator.q_node: np.array(q_node_gen),
                                                        self.discriminator.rel_node: np.array(rel_node_gen)})

                            feed_dict = {self.generator.q_node: np.array(q_node_gen), self.generator.rel_node: np.array(rel_node_gen),self.generator.reward: reward_gen}
                            _, loss, prob = self.sess.run([self.generator.gan_updates, self.generator.gan_loss, self.generator.i_prob],feed_dict=feed_dict)
                            all_score = self.sess.run(self.generator.all_score)
                            root_nodes = []
                            rel_nodes = []
                            root_nodes_gen = []
            ...
            for var, val in zip(tvars, tvars_vals):
                if "discriminator/discriminator/Variable_1:0" in var.name:
                    np.savetxt(config.emb_filenames[7], val)
                if "generator/Generator/Variable_1:0" in var.name:
                    np.savetxt(config.emb_filenames[6], val)
    ...
    def write_emb_to_txt(self,mode,num):
        """write the emd to the txt file"""
        #mode is either 0 or 1
        #num tells 1 of 4 config i.e. medianGen,medianDis,SoftmaxGen,SoftmaxDis  
        #num --- 0 -medianGen
        #num --- 1 -medianDis
        #num --- 2 -SoftmaxGen
        #num --- 3 -SoftmaxDis
        #num --- 5 -Gen-for every epoch(needed for accuracy calculation)
        #num --- 6 -Dis-for every epoch(needed for accuracy calculation)
        modes = [self.generator, self.discriminator]
        node_embed = self.sess.run(modes[mode].node_embed)
        a = np.array(range(self.n_node)).reshape(-1, 1)
        node_embed = np.hstack([a, node_embed])
        node_embed_list = node_embed.tolist()
        node_embed_str = ["\t".join([str(x) for x in line]) + "\n" for line in node_embed_list]
        with open(config.emb_filenames[num], "w+") as f:
            lines = [str(config.n_node) + "\t" + str(config.n_embed) + "\n"] + node_embed_str
            f.writelines(lines)
    ...
    def eval_test(self):
        ...
        if config.app == "citeseerNew":
            for i in range(2):
                LPE = elp.LinkPredictEval(config.emb_filenames[i+4], config.test_filename, config.test_neg_filename, config.n_node, config.n_embed,config.n_features,config.pretrain_feature_filename,config.emb_filenames[i+6],config.modes[i])
                result = LPE.eval_link_prediction()
                #results.append(config.modes[i] + ":" + str(result) + "\n")
                results.append(config.modes[i] + ":" + "median--"+str(result[0])+"--"+str(result[1])+"   softmax--"+str(result[2]) + "\n")
...
if __name__ == "__main__":
    #utils.generate_neg_links(config.train_filename, config.test_filename , config.test_neg_filename)
    g_g = GraphGan()
    g_g.train_gan()
