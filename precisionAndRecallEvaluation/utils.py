...
def readFeatureWt(filename):
    with open(filename, "r") as f:
        lines = f.readlines()
        featureWt = [float(line) for line in lines]
    return featureWt
...
def read_emd(filename, n_node, n_embed):
    ...
    if("ginifeature" in filename):
        for line in lines:
            emd = line.split("\t")
            node_embed[int(float(emd[0])), :] = [ bool(int(i)) for i in emd[1:]]
...
#readFeatureWt('GraphGAN/genFeatWeights.txt')
